...
def test_leaf_queries_retried(started_cluster):
    ...
    assert (
        int(
            node1.query(
                "SELECT max(ProfileEvents['DistributedShardTryCount']) FROM system.query_log"
            )
        )
        > 1
    )
...
def test_leaf_queries_not_retried(started_cluster):
    prepare_cluster()
    node1.query("SYSTEM ENABLE FAILPOINT remote_query_exception_after_receiving_data")
    query_id = str(uuid.uuid4())
    with pytest.raises(QueryRuntimeException, match="Injected CANNOT_OPEN_FILE error after receiving data"):
        node1.query("SELECT max(i) FROM distributed_table", query_id=query_id)    
    node1.query("SYSTEM FLUSH LOGS")
    assert (
        int(
            node1.query(
                f"SELECT max(ProfileEvents['DistributedShardTryCount']) FROM system.query_log WHERE query_id = '{query_id}'"
            )
        )
        == 0
    )
